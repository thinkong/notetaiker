...
export const hashtagHighlighter = ViewPlugin.fromClass(class {
...
- **Immediate promotion:** Don't move hashtags to frontmatter *while* typing; it creates noise in the document. Wait for the save event.
...
1. **How to handle tag removal?**
   ...
   - What's unclear: If they delete a `#tag` from the *body*, should it be removed from the frontmatter `tags` list on next save?
