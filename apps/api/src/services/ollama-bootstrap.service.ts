...
export class OllamaBootstrap {
  ...
  async ensure(): Promise<OllamaBootstrapResult> {
    ...
    if (fs.existsSync(binaryPath)) {
      console.log(
        `[ollama-bootstrap] Found portable Ollama at ${binaryPath}`,
      );
  ...
  private async downloadLatest(): Promise<string> {
    ...
    console.log("[ollama-bootstrap] Fetching latest release info from GitHub...");
  ...
  private async extractArchive(
    ...
    if (archivePath.endsWith(".zip")) {
      ...
      execSync(`tar --use-compress-program=unzstd -xf "${archivePath}" -C "${destDir}"`);
    } else {
