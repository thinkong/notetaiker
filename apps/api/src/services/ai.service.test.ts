...
describe('AIService', () => {
  ...
  describe('provider selection', () => {
    it('should select OpenAI if apiKey is present', async () => {
      ...
      (generateObject as any).mockResolvedValue({ object: { tags: ['A', 'B', 'C'] } });
    ...
    it('should select Anthropic if OpenAI is missing and Anthropic is present', async () => {
      ...
      expect(createAnthropic).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'ant-test' }));
    });
    ...
    it('should select Gemini if others are missing and Gemini is present', async () => {
      ...
      expect(createGoogleGenerativeAI).toHaveBeenCalledWith(expect.objectContaining({ apiKey: 'gem-test' }));
    });

    it('should throw error if no provider is configured', async () => {
      secretsServiceMock.getSecrets.mockResolvedValue({});

      await expect(aiService.generateTags('test content')).rejects.toThrow('No AI provider configured');
    });
  });
  ...
  describe('generateTags', () => {
    it('should return tags from generateObject', async () => {
      ...
      const { generateObject } = await import('ai');
      (generateObject as any).mockResolvedValue({ object: { tags: expectedTags } });
